  ...
  useEffect(() => {
    if (Object.keys(errors).length === 0 && isSuccess) {
      async function signinAPI() {
        await axios
          .post(SIGNIN, {
            username: formDatas.signInEmail,
            password: formDatas.signInPassword,
          })
          .then((response) => {
          ...
          .catch((error) => {
            if (error.code === "ERR_NETWORK") {
              SetSignInError({
                networkError:
                  "sorry for inconvineant we face some network issues,please try after some time!!!",
              });
              ...
              SetSignInError({
                username:
                  "The user does not exist! check the username or register",
              });
            }
          });
      }
      ...
      <div className="signin">
        <div className="signin-container">
          <div className="signin-container-items item1">
            <p>
              <b>Welcome to the amazing world of Unipick!</b>
            </p>
          ...
          <p>
            Your gateway to all things sustainable, creative, cultural and
          ...
          <div className="signin-container-items item2">
            ...
            <IconContext.Provider value={{ color: "#fff" }}>
              <span className="signin-icons whatsapp">
                {" "}
                <BSIcons.BsWhatsapp />{" "}
              </span>
            </IconContext.Provider>
            <span className="signin-icons">
              {" "}
              <FCIcons.FcGoogle />{" "}
            </span>
          ...
          {isSiginUp ? (
            ...
            <>
              <form
                className="signin-container-items item3"
              ...
              >
                ...
                <div className="form-password">
                  <div className="password-items1">
                    <input
                      ...
                      name="signInPassword"
                      value={formDatas.signInPassword}
                      onChange={handleChange}
                    ></input>
                    {visible ? (
                  ...
                  {visibileError ? (
                    <p className="form-pass-errors">{errors.password}</p>
                  ) : (
                    ""
                  )}
                </div>
                ...
                {signinError.networkError ? (
                  <p className="form-errors">{signinError.networkError}</p>
                ) : (
                  ""
                )}
                {signinError.username ? (
                  <p className="form-errors">{signinError.username}</p>
                ) : (
                  ""
                )}
                {signinError.incorrect ? (
                  <p className="form-errors">{signinError.incorrect}</p>
                ) : (
                  ""
                )}
              ...
              <div className="signin-container-items item4">
                By clicking Sign in or Sign in with Google or Facebook or
                WhatsApp, you agree to Unipick's Terms of Use and Privacy
                Policy. Unipick may send you communications. You may change your
